...
const ChatScreen: React.FC<Props> = ({ route, navigation }) => {
  ...
  useEffect(() => {
    ...
    const fetchMessagesPrivate = async () => {
      ...
      const privateMessageArray = JSON.parse(privateMessage) as {
        [key: string]: Message[];
      };
    ...
    const handleCustomEvent = async (buffer: Uint8Array) => {
      ...
      if (packet.networkId == "text_packet") {
        ...
        if (
          textPacket.type == TextPacketType.TYPE_CHAT ||
          textPacket.type == TextPacketType.TYPE_BROADCAST_CHANEL
        ) {
          ...
          privateMessageArray[_private] = [
            ...privateMessageArray[_private],
            {
              ...
              parameters: textPacket.parameters,
              type: textPacket.type,
            } as Message,
          ];
          await AsyncStorage.setItem(
            "private",
            JSON.stringify(privateMessageArray),
          );
          if (_private != user) {
            return;
          }
        ...
        const joinChannelPacket: IJoinChannelPacket =
          packet as IJoinChannelPacket;
  ...
  const handleSend = () => {
    ...
    const newMessageObject = {
      ...
      type: groupName != undefined
        ? TextPacketType.TYPE_CHAT
        : TextPacketType.TYPE_PRIVATE_CHAT,
