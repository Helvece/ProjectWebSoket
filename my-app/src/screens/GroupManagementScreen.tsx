  ...
  useEffect(() => {
    ...
    const fetchGroups = async () => {
      socket.emit(
        "packet",
        await encodeAsync({
          networkId: "get_list_channel_packet",
          type: ["public", "whitelisted"],
        }),
      );
    };
    const handleCustomEvent = async (buffer: Uint8Array) => {
      ...
      if (packet.networkId === "status_channel_packet") {
        const statusPacket: IStatusChannelPacket =
          packet as IStatusChannelPacket;
        if (
          statusPacket.type == ChannelType.PUBLIC ||
          statusPacket.type == ChannelType.WHITELISTED
        ) {
          if (statusPacket.status === "add") {
            setGroups((
              ...
              name: statusPacket.channel,
            }]);
          } else if (statusPacket.status === "remove") {
            setGroups((
              prev,
            ) => [
              ...prev.filter((group) => group.name !== statusPacket.channel),
            ]);
          }
        }
        ...
        let id = 0;
        setGroups((prev) =>
          listPacket.channels.map((channel) => ({
            id: String(id++),
            name: channel,
          }))
        );
  ...
  const CreateNewGroup = async (groupName: string) => {
    ...
    try {
      getSocket().emit(
        "packet",
        await encodeAsync({
          networkId: "create_channel_packet",
          name: groupName,
          persistent: true,
          permission: ["*"],
        }),
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.groupCreationContainer}>
        ...
        />
        <TouchableOpacity
          onPress={() => CreateNewGroup(newGroupName)}
